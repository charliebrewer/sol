...
var OrbitalMechanics = function() { // TODO need to use browserify to share code between the client and server
	...
	module.EARTH_YEAR_PERIOD      = 60 * 60; // In game seconds that the Earth takes to orbit the sun
...
app.ticker.add(function(delta) {
	...
	if(waitPeriodHolder < waitPeriodSeconds) {
		...
		if(currentZoom != desiredZoom) {
			currentZoom += (desiredZoom - currentZoom) / 100;
		}
		
		for(var i = 0; i < celestialBodies.length; i++) {
			celestialBodies[i].sprite.x = (((currentZoom / 10) * ((celestialBodies[i]['pos'][0] / (2 * CENTER_OF_SYSTEM)) - 0.5)) + 0.5) * app.renderer.width;
			celestialBodies[i].sprite.y = (((currentZoom / 10) * ((celestialBodies[i]['pos'][1] / (2 * CENTER_OF_SYSTEM)) - 0.5)) + 0.5) * app.renderer.height;
		}
	}
});

document.addEventListener("mousewheel", function(e) {
	var dir = 2 * Math.round(e.wheelDelta / Math.abs(e.wheelDelta)); // Set the dir value to 1 or -1
	
	desiredZoom = Math.max(10, desiredZoom + dir);
}, false);
/*
