...
router.post('/', function(req, res, next) {
	var output = {"responseCode" : RESPONSE_FAILURE, "message" : "unknown error", "data" : {}};

	var error = true;
	
	if(!req.body.req) {
		output.message = "req was not defined";
	} else {
		// TODO add exception handling for invalid JSON
		var request = JSON.parse(req.body.req);
		
		if(!request.command) {
			output.message = "no command specified"
		} else {
			...
			if(!command) {
				...
				CommandController().runCommand(command, request.data, output, function(output){
					res.json(output);
				});
			}
		}
